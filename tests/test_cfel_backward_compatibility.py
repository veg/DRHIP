...
def _load_cfel_results(comparison_results_dir: str, gene_basename: str) -> Dict[str, Any]:
    ...
    if gene_basename.endswith('.CONTRASTFEL') or gene_basename.endswith('.CONTRASTFEL.no-subs'):
...
def test_cfel_validation_passes_without_substitutions(comparison_results_dir: str):
    ...
    assert 'substitutions' not in results, "Test data should not have substitutions field"
...
def test_cfel_process_comparison_data_without_substitutions(comparison_results_dir: str):
    """
...
def test_cfel_process_comparison_site_data_without_substitutions_warns(comparison_results_dir: str):
    """
...
def test_cfel_process_comparison_site_data_without_substitutions_has_core_fields(comparison_results_dir: str):
    """
...
def test_cfel_comparison_with_and_without_substitutions(comparison_results_dir: str):
    ...
    gene_base = "pretend_DENV1_ref.part_NC_001477.1__capsid_protein_C__95-394_DENV1.CONTRASTFEL"
    
    results_with_subs = _load_cfel_results(comparison_results_dir, gene_base)
    results_without_subs = _load_cfel_results(comparison_results_dir, f"{gene_base}.no-subs")
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        site_data_without = method_without.process_comparison_site_data(results_without_subs)
...
def test_cfel_no_regression_on_newer_files(comparison_results_dir: str):
    ...
    """
    gene_basename = "pretend_DENV1_ref.part_NC_001477.1__capsid_protein_C__95-394_DENV1.CONTRASTFEL"
    results = _load_cfel_results(comparison_results_dir, gene_basename)
